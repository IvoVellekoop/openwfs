...
class KCubeInertial(Actuator):
    ...
    def __init__(self, serial_number: str = "", pair_channels: bool = False, timeout: u.Quantity = 20*u.s):
        ...
        if not th_ine_lib_found:
            raise ImportError("Thorlabs Kinesis libraries not found. Download and install Thorlabs Kinesis software from https://www.thorlabs.com/software-pages/motion_control/.")
        ...
        if serial_number == "":
            if len(serial_number_list) == 1:
                ...
                raise ValueError(f"Multiple KCube Inertial Motor devices found. Please specify a serial number. Available devices: {serial_number_list}")
            else:
