...
class SimulatedWFS:
    ...
    def wait(self):
        "The time where the image gets created and put in the pre-existing buffer"
        E_field_slm = self.E_input_slm * np.exp(1j * (((self.SLM_pattern - self.ideal_wf) / 256) * 2 * np.pi))
        E_field_slm_F = np.fft.fft2(E_field_slm)

        image_plane = np.array(abs(np.fft.fftshift(E_field_slm_F) ** 2))
    ...
    def set_data(self, pattern):
        pattern = cv2.resize(pattern, dsize=np.shape(self.SLM_pattern), interpolation=cv2.INTER_NEAREST)
        self.SLM_pattern = pattern
        if self._active_plotting:
            self.slm_plot.set_data(self.SLM_pattern % 256)
            plt.pause(0.001)
            plt.draw()
...
def generate_double_pattern(shape, phases_half1, phases_half2, phase_offset):
    ...
    if phases_half1 == 0:
        ...
        half1 = [[int(((i + phase_offset) / (width / phases_half1)) * 256) % 256 for _ in range(half_width)] for i in range(height)]
