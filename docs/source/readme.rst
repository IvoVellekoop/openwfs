...
Wavefront shaping (WFS) is a technique for controlling the propagation of light in arbitrarily complex structures, including strongly scattering materials :cite:`kubby2019`. In WFS, a spatial light modulator (SLM) is used to shape the phase and/or amplitude of the incident light. With a properly constructed wavefront, light can be made to focus through :cite:`Vellekoop2007`, or inside :cite:`vellekoop2008demixing` a scattering material in such a way that the light interferes constructively at the desired focus; or light can be shaped to have other desired properties, such as optimal sensitivity for specific measurements :cite:`bouchet2021maximum`, specialized point-spread functions :cite:`boniface2017transmission` or for functions like optical trapping :cite:`vcivzmar2010situ`.
...
* :attr:`~.WFSResults.fidelity_amplitude`: The fidelity reduction due to unequal illumination of the SLM. Note: for phase-only correction of a strongly scattering sample with a gaussian illumination profile, the theoretical maximum of this fidelity reduces to π/4. :cite:`goodman2015statistical`.
* :attr:`~.WFSResults.fidelity_calibration`: The fidelity reduction due to imperfect phase response calibration of the SLM.
...
* :attr:`~.WFSTroubleshootResult.fidelity_non_modulated`: The fidelity reduction due to non-modulated light.
...
Lastly, the :func:`~.troubleshoot` function computes several image frame metrics:

* :attr:`~.WFSTroubleshootResult.frame_cnr_after`: The unbiased Contrast to Noise Ratio (CNR) of a frame captured after running the algorithm. The noise is estimated from a dark frame measurement.
* :attr:`~.WFSTroubleshootResult.contrast_enhancement`: The unbiased contrast enhancement due to WFS. A frame taken with a flat wavefront is compared to a frame taken with a shaped wavefront. Both frames are taken after the WFS algorithm has run. This metric is especially useful for scenarios where the contrast is expected to improve due to WFS, such as in multi-photon excitation fluorescence (multi-PEF) microscopy.

and :func:`~.troubleshoot` tests:

* :attr:`~.WFSTroubleshootResult.frame_repeatability`: Pearson correlation of two frames taken before running the WFS algorithm, with a flat wavefront. Not compensated for noise.
* :attr:`~.WFSTroubleshootResult.stability`: A ``StabilityResult`` object containing the result of a stability test.
